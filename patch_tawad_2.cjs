...
// Insert the SubTool toggle below the main title.
const toggleHTML = `
...
// We need to close the ternary at the end of the return block and provide the freelance calculator.
const calcHTML = `
        </>
      ) : (
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-blue-600 dark:bg-blue-600/20 p-6 rounded-3xl border border-blue-500 shadow-lg shadow-blue-500/20 text-white relative overflow-hidden h-full flex flex-col justify-center">
              ...
              {(() => {
                ...
                // Total target net income per month
                const totalMonthlyNeed = expenses + internet + hardware;
                // Gross income needed to cover tax
                ...
                const hourlyRateUSD = hourlyRate / 58; // approx USD conversion
...
// We need to fix the UI rendering for the 'salary_offer' labels.
// We can change the labels dynamically based on `scenario === 'salary_offer'` 
content = content.replace(
